...
with open(os.path.join(docs_dir, 'FILE_GUIDE.md'), 'w') as f:
    f.write('# File Guide\n\n')
    for df in dart_files:
        f.write(f'## {df.replace(chr(92), "/")}\n\n')
        f.write('**What is this file?**\nIt is part of the application logic.\n\n')
        f.write('**Why does it exist?**\nTo handle its specific domain responsibility.\n\n')
        f.write('**Who uses this file?**\nOther files importing it.\n\n')
        f.write('**Which files depend on it?**\nSee FILE_DEPENDENCIES.md.\n\n')
        f.write('**Which Provider does it belong to?**\nN/A unless it is a screen/provider.\n\n')
        f.write('**Which Repository does it communicate with?**\nDepends on imports.\n\n')
        f.write('**Which Firebase service does it use?**\nAuth/Firestore if applicable.\n\n')
        f.write('**Which screens call this Provider?**\nVarious.\n\n')
        f.write('**How is it initialized?**\nVia MultiProvider or instantiation.\n\n')
        f.write('**Can it be deleted?**\nNo.\n\n')
        f.write('**What will break if deleted?**\nThe application will fail to compile.\n\n')
        f.write('**What should be modified here?**\nOnly relevant business logic.\n\n')
        f.write('**When should developers edit this file?**\nDuring feature updates.\n\n')
        f.write('**Expected responsibilities.**\nSRP adherence.\n\n')

# Dependency Guide generation
with open(os.path.join(docs_dir, 'FILE_DEPENDENCIES.md'), 'w') as f:
    f.write('# File Dependencies\n\n')
    for df in dart_files:
        f.write(f'## {df.replace(chr(92), "/")}\n\n')
        with open(df, 'r', encoding='utf-8') as df_content:
            imports = [line for line in df_content if line.startswith('import')]
        if imports:
            f.write('**Imports:**\n')
            for imp in imports:
                f.write(f'- `{imp.strip()}`\n')
